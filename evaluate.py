...
def main():
    ...
    val_loader = torch.utils.data.DataLoader(
        img_segmentation.datasets.VOC2011ClassSeg(
    ...
    if osp.basename(model_file).startswith('fcn32s'):
        model = img_segmentation.models.FCN32s(n_class=21)
    elif osp.basename(model_file).startswith('fcn16s'):
        model = img_segmentation.models.FCN16s(n_class=21)
    elif osp.basename(model_file).startswith('fcn8s'):
        if osp.basename(model_file).startswith('fcn8s-atonce'):
            model = img_segmentation.models.FCN8sAtOnce(n_class=21)
        else:
            model = img_segmentation.models.FCN8s(n_class=21)
    else:
        raise ValueError
    ...
    for batch_idx, (data, target) in tqdm.tqdm(enumerate(val_loader),
        ...
        #         viz = img_segmentation.utils.visualize_segmentation(
        #             lbl_pred=lp, lbl_true=lt, img=img, n_class=n_class,
        #             label_names=val_loader.dataset.class_names)
        #         visualizations.append(viz)
    metrics = img_segmentation.utils.label_accuracy_score(
...
FWAV Accuracy: {3}'''.format(*metrics))
    ...
    # viz = img_segmentation.utils.get_tile_image(visualizations)
